...
public class BTC extends BaseCoin implements BaseCoinI {
    ...
    public void ExtendBaseInfo(YangCurrency yangCurrency, RedisService redisService) throws LogException {
        ...
        try {
            InetAddress ip= InetAddress.getByName("bcc-coin-service");
            String host = ip.getHostAddress();
            URL url = new URL("http://wang628625:628625@"+host+":8332");
//            URL url = new URL("http://wang628625:628625@47.75.155.204:8335");
    ...
    public String sendTransaction(String from, long num, String to, String passwd) throws LogException {
        try {
            if(this.unlock())
            {
                String hash =  this.bitcoinJSONRPCClient.sendToAddress(to,num);
                this.lock();
                return hash;
            }else{
    ...
    public String sendTransaction2cold(String from, double num, String passwd) throws LogException {
        String coldAddress = "1LtLn8ctDsvBhyL76h6Nm5i9gLPL8LBeeq";
        // TODO: 2018/6/6 开始测试
        try {

            if(this.unlock())
            {
                System.out.println("开始转账，冷钱包地址为:1LtLn8ctDsvBhyL76h6Nm5i9gLPL8LBeeq 金额："+num);
...
//            this.lock();
    ...
    public void confirm(BigInteger blockNumber)
    {
        try {
            ...
            Result result = memberService.confirm(blockNumber, CurrencyIds,6);
            if(result.getCode().equals(Result.Code.SUCCESS) && result.getData() != null) {
                ...
                memberService.finishConfirm(blockNumber, o1.toString(), o2.toString(), 6);
                //然后和服务端再次教研 改变状态
            }
    ...
    public void safe() {
        ...
        try{
            ...
            BigDecimal needLeft = BigDecimal.valueOf(0.1);
    ...
    public static void main(String[] args)
    {
        ...
        String password2 = CommonTools.Md5("453518969@qq.com");
