...
public class KlineTask implements Runnable{
    ...
    public KlineTask(RedisService redisService,MongoTemplate mongoTemplate,YangTradeService yangTradeService){
    ...
    public void run() {
        while(true){
            try{
                ...
                int size=redisService.listSize(CoinConst.KLINE_TRADE_DATA).intValue();//KLINE_TRADE_DATA  k线数据
                //判断 交易记录缓存是否存在
                if(size>0){
...
//                    Collections.sort(tradeList,new YangTradeCompareByAddTime());
                    ...
                    if(alist.size()>0){
                        ...
                        for (int i=0;i<alist.size();i++) {
                            ...
                            Map<String, Object> mongoMap = KlineTask.getMongoDataId(times);
...
//                mg.setTradeId(Integer.valueOf(ft.getTradeId()));
            ...
            yangTradeService.saveBatch(mgList, CoinConst.MONGODB_TRADE_COLLECTION);
        }
    ...
    public static void main(String[] args) throws Exception{
        ...
        System.out.println( KlineTask.dayForWeek(DateUtils.getDataStampStr(1535971293L)));
        System.out.println( KlineTask.getMongoDataId(1535971293L));
