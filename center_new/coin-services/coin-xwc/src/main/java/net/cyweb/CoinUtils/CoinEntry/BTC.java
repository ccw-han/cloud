...
public class BTC extends BaseCoin implements BaseCoinI {
    ...
    public void ExtendBaseInfo(YangCurrency yangCurrency, RedisService redisService) throws LogException {
        ...
        try {
             InetAddress ip= InetAddress.getByName("xwc-coin-service");
            String address = ip.getHostAddress();
            URL url = new URL("http://infinitecoinrpc:4p9UZojd3t8h9XF4ZWGqDkqhFthzL6xVNnXMUhGVrPZ9@"+address+":18332");
            System.out.println("连接地址为"+"http://infinitecoinrpc:4p9UZojd3t8h9XF4ZWGqDkqhFthzL6xVNnXMUhGVrPZ9@"+address+":18332");


//            URL url = new URL("http://infinitecoinrpc:4p9UZojd3t8h9XF4ZWGqDkqhFthzL6xVNnXMUhGVrPZ9@47.75.155.204:18332");

            this.bitcoinJSONRPCClient = new BitcoinJSONRPCClient(url);
            System.out.println(bitcoinJSONRPCClient.getBlockCount());
    ...
    public String sendTransaction2cold(String from, double num, String passwd) throws LogException {
        String coldAddress = "WXnvsNFq7NejWdncKmkfaA5NKWMrkmwSjW";
        try {
            if(this.unlock())
            {
...
//        this.bitcoinJSONRPCClient.getRawTransaction("e37b7d66e12673ed2da6d505210a0fe444d128bd9bffb8561d33f2b2492f04ec").confirmations()
        for (String tx : txList
                ) {
            try {
                Bitcoin.RawTransaction rawTransactionbase = this.bitcoinJSONRPCClient.getTransaction(tx);
                if(rawTransactionbase.vOut().size() > 2 && rawTransactionbase.vOut().get(0).value() == Double.valueOf(0))
                {
                    continue;
                }
     ...
     */
    @Async
    public void confirm(Bitcoin.Block block)
    {
        try {

            System.out.println("开始确认之前充币是否到账---");

            BigInteger blockNumber = BigInteger.valueOf(block.height());
            ...
            if(result.getCode().equals(Result.Code.SUCCESS) && result.getData() != null) {
                ...
                memberService.finishConfirm(blockNumber, o1.toString(), o2.toString(), 10);
                //然后和服务端再次教研 改变状态
            }
    ...
    public void safe() {
        ...
        try{
            ...
            BigDecimal needLeft = BigDecimal.valueOf(1000);
    ...
    {
        Object o =  this.bitcoinJSONRPCClient.query("walletpassphrase","wang628625",10);
        if(o == null)
        {
            return true;
        }else{
